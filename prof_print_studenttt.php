<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>View</title>
    <link href="https://fonts.googleapis.com/css2?family=Poppins&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="bg.css">
    <link href='https://css.gg/log-out.css' rel='stylesheet'>
    <script src="https://kit.fontawesome.com/faeaa9a8c9.js" crossorigin="anonymous"></script>
</head>
    <nav>
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
    <nav>
        <ul>
          <li><a href='viewtt_prof_stud.php'> &laquo Back</a></li>
         </ul>
       </nav>
</div>
    </form>
    </nav>
        <!-- logo -->
            <div class="logo"><span><img src="Nitc_logo.png"> </span></div>
<style>
<?php include 'tt.css'; ?>

</style>
</html>
<?php 
    session_start();
    include 'dbconnection.php';
    $conn = OpenCon();
    $_SESSION["cday"] = flush_database($conn,$_SESSION["cday"]);
    
    $prof=$_SESSION["pid"];
    $selected=$_SESSION["selectedbatch"];

    $days = array("Monday","Tuesday","Wednesday","Thursday","Friday");
    $all_slots = array("8-9 AM","9-10 AM","10:15-11:15 AM","11:15-12:15 PM","1-2 PM","2-3 PM","3-4 PM","4-5 PM");

    echo "<html><body><center><justify><table class='table1'cellspacing=0 cellpadding=0>";
    echo "<h1>STUDENT TIMETABLE</h1>";
    echo '<tr><center>';
    echo '<th class="days"><center>' . "DAY/SLOT" . '</center></th>';

    foreach($all_slots as $s) 
    {
        echo '<th class="days"><center>' . $s . '</center></th>';
    }
    echo '</center></tr>';
    
    foreach($days as $day)
    {
        echo '<tr">';
        echo '<td  class="days">' . $day. '</td>';
        $cl=0;
        for($slotid=1;$slotid<=8;$slotid++)
        {
            $sql="select course_id,room,building from weeklyTable as w,classroom as c where w.room_id = c.room_id and batch_id=? and day=? and slot_id=?";
            $q2=$conn->prepare($sql);
            $q2->bind_param("dsd",$selected,$day,$slotid);
            $q2->execute();
            $q2->bind_result($course_id,$room,$building);
            $q2->store_result();
            $flag=0;
            $course_array=array();
            while($q2->fetch())
            {
                array_push($course_array,array($course_id,$building,$room));
                $flag=1;
            }
            if($flag==0)
            {
                echo '<td  class="t2">' ."-". '</td>';
            }
            else
            {
                echo '<td class="t2"><center>';
                foreach($course_array as $c)
                {
                    echo $c[0];
                }
                echo '</center></td>';
            }
        }
        echo '</tr>';
    }

?>